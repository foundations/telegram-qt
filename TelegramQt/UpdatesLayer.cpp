...
Q_LOGGING_CATEGORY(c_clientUpdatesLayerCategory, "telegram.client.rpclayer.updates", QtDebugMsg)

namespace Telegram {

namespace Client {

UpdatesLayer::UpdatesLayer(QObject *parent) :
    QObject(parent)
{
}

void UpdatesLayer::setBackend(Backend *backend)
{
    m_backend = backend;
}

bool UpdatesLayer::processUpdates(const TLUpdates &updates)
{
    qCWarning(c_clientUpdatesLayerCategory) << "updates:" << updates;

    switch (updates.tlType) {
    case TLValue::UpdatesTooLong:
        qCDebug(c_clientUpdatesLayerCategory) << "Updates too long!";
//        getUpdatesState();
        break;
        ...
        if (!updates.updates.isEmpty()) {
            ...
            for (int i = 0; i < updates.updates.count(); ++i) {
                if (updates.updates.at(i).pts < pts) {
                    qCDebug(c_clientUpdatesLayerCategory) << "Unordered update!";
                    //Q_ASSERT(0);
                }
...
bool UpdatesLayer::processUpdate(const TLUpdate &update)
{
...
//        }
    ...
    {
//        qCDebug(c_clientUpdatesLayerCategory) << Q_FUNC_INFO << "UpdateNewMessage";
...
MessagingApi *UpdatesLayer::messagingApi()
{
    return m_backend->messagingApi();
}

DataStorage *UpdatesLayer::dataStorage()
{
